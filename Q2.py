...
def check_and_divide(filename = "time_series.csv"):
    ...
    with open("time_series.csv" , newline= '' , encoding= "utf-8" )as csvfile:
        ...
        for row in reader:
            ...
            double.add(date_obj) #אם התאריך לא נמצא עדיין - הכנסתו לטבלה

            rows_per_hour[date_obj.hour].append(row) #כל תאריך שנמצא תקין נכנס למילון כאשר המפתח הוא השעה שלו
